...
function rollDice() {
  ...
  const rollPromises = dice.map(rollDie);
  return Promise.race(rollPromises);
}
...
// Because the Promice.race() gives us the first resolved promise, I think the others keep running behind
// and because it didn't wait for others to resolve we got their results later.
