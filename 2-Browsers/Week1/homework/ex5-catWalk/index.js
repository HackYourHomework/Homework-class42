...
function catWalk() {
  ...
  if (currentLeft > window.innerWidth - image.width) {
    image.style.left = 0;
  } else if (
    parseInt(image.style.left) >= (window.innerWidth - image.width) / 2 &&
    parseInt(image.style.left) < window.innerWidth / 2
  ) {
    ...
    setTimeout(() => {
      ...
      image.style.left = currentLeft + image.width / 2 + 'px';
    }, 5000);
  } else {
