...
function computeEarnings(tasks, rate) {  
  const timeDuration = tasks.map(time => {return time.duration})
  const bill = timeDuration.map(element => {return element * rate / 60}) 
  // divide to 60 because duration specified to minutes but we need hourly bill 
  const totalBill = bill.reduce((acc, item) => {return acc + item}, 0);  
