...
// ! Function under test
function sanitizeFruitBasket(fruitsToBeSanitized, hazardousFruit) {
  const newFruitBasket = fruitsToBeSanitized.filter(
    (fruits) => fruits !== hazardousFruit
  );
  return newFruitBasket;
}

//console.log(sanitizeFruitBasket(fruitBasket, 'lemon')); in order to test the function
...
describe('sanitizeFruitBasket', () => {
  ...
  test('should not modify the original `fruitBasket` array', () => {
    ...
    expect(originalFruitBasketContents).toStrictEqual(expected);
  });
